...
void Guard::TakeDamage(short amount)
{
    Entity::TakeDamage(amount);
    if (health <= 0)
        Notify(this, Event::GuardDefeated);
}
...
void Guard::SearchPrisoner(Entity *target)
{
    if (!target)
        return;
    ...
    else if (auto i = dynamic_cast<Inmate *>(target))
        targetInv = &i->GetInventory();
    if (targetInv)
    {
        std::cout << "[Guard] " << name << " is searching " << target->GetName() << ".\n";
        auto items = targetInv->GetItems();
        ...
        for (const auto &item : items)
        {
            ...
            {
                confiscatedItems.AddItem(item);
                foundContraband = true;
            }
            else
                keptItems.push_back(item);
        }
        targetInv->SetItems(keptItems);
        if (foundContraband)
        {
            std::cout << "  -> Found contraband! Confiscated.\n";
            if (auto p = dynamic_cast<Player *>(target))
                p->SetHeat(100);
        }
    }
...
void Guard::Update(float deltaTime, const PrisonMap &map)
{
    ...
    if (aggroPlayer)
    {
        ...
        else if (dist > 20.f)
        {
            ...
            if (currentPath.empty() ||
                RandomGenerator::GetInstance().GetInt(0, 59) == 0)
    ...
    else// random path
    {
        if (currentPath.empty())
        {
            ...
            {
                patrolTimer = 1.5f + static_cast<float>(
                                         RandomGenerator::GetInstance().GetInt(0, 99)) /
                                         50.f; // wait time
                ...
                while (!goodposition)
                {
                    ...
                    if (hasTargetZone)
                    {
                        ...
                        tileX =
                            startX +
                            (width > 0 ? RandomGenerator::GetInstance().GetInt(0, width - 1)
                                       : 0);
                        tileY = startY +
                                (height > 0
                                     ? RandomGenerator::GetInstance().GetInt(0, height - 1)
                                     : 0);
                    }
                    else
                    {
                        tileX = 2 + RandomGenerator::GetInstance().GetInt(
                                    0, (map.GetWidthInPixels() / 16) - 5);
                        tileY = 2 + RandomGenerator::GetInstance().GetInt(
                                    0, (map.GetHeightInPixels() / 16) - 5);
                    }
                    cx = static_cast<float>(tileX) * 16.f + 8.f;
                    cy = static_cast<float>(tileY) * 16.f + 8.f;
                    if (map.IsSolidWall(cx, cy))
                        goodposition = false;
                }
